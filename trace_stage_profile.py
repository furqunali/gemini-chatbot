...
def profile_trace_stages(trace: ResponseTrace) -> TraceStageProfile:
    ...
    return TraceStageProfile(
        tuple(sorted(stage_counts)),
        len(trace.events),
        tuple(sorted(s for s, n in stage_counts.items() if n > 1)),
        tuple(sorted(status_counts)),
        sum(event.status.lower() in {"failed", "error"} for event in trace.events),
    )
